...
import cn.seecoder.courselearning.serviceimpl.order.rentStrategy.BaseRentStrategy;
import cn.seecoder.courselearning.serviceimpl.order.rentStrategy.RentStrategy;
...
public class CourseOrderServiceImpl implements CourseOrderService {
    ...
    public ResultVO<CourseOrderVO> updateCourseOrder(Integer orderId, Integer orderStatus) {
        ...
        if ((orderStatus.equals(Constant.ORDER_STATUS_SUCCESS) || orderStatus.equals(Constant.ORDER_STATUS_WAIT)) &&
            ...
            if (user.getBalance() >= order.getCost()) {
                // 如果是租用类型订单，找到endTime最新的一个订单，重新计算租用结束时间存入数据库
                if (order.getPurchaseType().equals(PurchaseType.Rent)) {
                    CourseOrder hadRentCourseOrder = orderMapper.selectEndTime(order);
                    if (null != hadRentCourseOrder && hadRentCourseOrder.getEndTime().after(new Date())) {
                        order.setEndTime(new Date(order.getEndTime().getTime() - order.getCreateTime().getTime() + hadRentCourseOrder.getEndTime().getTime()));
                    } else {
                        order.setEndTime(new Date(order.getEndTime().getTime() - order.getCreateTime().getTime() + System.currentTimeMillis()));
                    }
    ...
    public ResultVO<CourseOrderVO> createCourseOrder(Integer courseId, Integer userId) {
        ...
        order.setPurchaseType(PurchaseType.Buy);
    ...
    @Override
    public ResultVO<CourseOrderVO> createCourseRentOrder(OrderRentDTO orderRentDTO) {
        List<CourseOrder> courseOrderList = orderMapper.selectByUserId(orderRentDTO.getUserId());

        for (CourseOrder order : courseOrderList) {
            if (order.getCourseId().equals(orderRentDTO.getCourseId())) {
                if (order.isBought())
                    return new ResultVO<>(Constant.REQUEST_FAIL, "已购买该课程");
            }
        }
        RentStrategy rentStrategy = new BaseRentStrategy();
        CourseOrder order = orderMapper.queryMostRecentOrder(orderRentDTO.getUserId(), orderRentDTO.getCourseId());
        if (null == order) {
            order = new CourseOrder();
        }
        Course course = courseService.getByPrimaryKey(orderRentDTO.getCourseId());

        order.setUserId(orderRentDTO.getUserId());
        order.setCourseId(orderRentDTO.getCourseId());
        order.setPurchaseType(PurchaseType.Rent);
        order.setCourseName(course.getName());
        order.setStatus(Constant.ORDER_STATUS_UNPAID);

        order.setCreateTime(new Date());
        order.setEndTime(new Date(System.currentTimeMillis() + (long)orderRentDTO.getTime() * 24 * 3600 * 1000));

        order.setOrigin(null);
        order.setCost(rentStrategy.calRentMoney(course.getCost(), orderRentDTO.getTime()));

        orderMapper.insert(order);

        return new ResultVO<>(new CourseOrderVO(order));
    }
