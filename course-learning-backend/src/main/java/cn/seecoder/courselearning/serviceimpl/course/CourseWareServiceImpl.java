...
public class CourseWareServiceImpl implements CourseWareService {
    ...
    public CourseWareVO getCourseWare(Integer uid, Integer courseWareId) {
        ...
        if (uid != null && uid > 0){
            ...
            if(order != null) {
                isAvailable = order.isOk();
            }
        }
        return new CourseWareVO(courseWare, isAvailable);
    }
    ...
    public List<CourseWareVO> getAllCourseWare(Integer uid, Integer courseId) {
        ...
        if (uid != null && uid > 0){
            CourseOrder order = orderService.queryMostRecentOrder(uid, courseId);
            if(order != null)
                isAvailable = order.getStatus().equals(Constant.ORDER_STATUS_SUCCESS);
        }
        ...
        for(CourseWare courseWare: tempList){
            ret.add(new CourseWareVO(courseWare, isAvailable));
        }
        return ret;
    }
